...
/* exactly 4
  ...
  your fix: i replaced the "... or equal to" operators with "greater than" and "less than"
    this allows the number 4 to skip over the first two paths and go through the `else`
...
/* experiments
  ...
  2. change the second comparison in the same way
    predict: similar to the first experiment, if i change the second condition to >
      then the next path will be executed.  the next path is `else` so it should be perfect
    actual: 'abcd' -> 'perfect!'
    why: the first experiment fixed the first check, the second experiment fixed the second path
      both paths had a similar mistake so it was basically the same fix
